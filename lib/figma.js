...
function containsHyperlink(checkNode) {
	if (
		...
		// console.log('gotta link from: ' + checkNode.name);
		return true;
	} else {
		// console.dir(checkNode.styleOverrideTable);
		if (
			...
			for (const key of Object.keys(checkNode.styleOverrideTable)) {
				// console.dir(checkNode.styleOverrideTable[key]);
				if (
...
function checkForLinksRecursive(checkNodeRecursive, found = []) {
	// console.log(checkNodeRecursive.name);
	if (containsHyperlink(checkNodeRecursive)) {
		// console.log('got a link from: ' + checkNodeRecursive.name);
...
const createComponent = (component, svgMap, imageFills, componentMap) => {
	...
	const visitNode = (node, parent, lastVertical, indent) => {
		...
		if (parent != null) {
			...
			//if it's the first child; aka first level frame in Figma
		...
		// here's where it does ordering with z-index; does this break everything lol?
		// or can I do a check here if the node contains a link?
		// }

		// if (containsLink) { //if it's top-level
		// 	console.log(node.name + ' containsLink true');
		// 	styles.zIndex = 21;
		// }
		...
		if (
			...
			if (['FRAME', 'COMPONENT', 'INSTANCE', 'GROUP'].indexOf(node.type) >= 0) {
				...
				if (lastFill) {
					if (lastFill.type === 'SOLID') {
						...
						styles.backgroundImage = 'url(' + imageFills[lastFill.imageRef] + ')';
			...
			// 	content = [`<a href="${node.name}" >${node.characters} </a>`];
			// }
		}
		...
		if (node.id !== component.id && node.name.charAt(0) === '#') {
			...
			for (const child of minChildren) {
				...
				//this is where it checks each Frame
