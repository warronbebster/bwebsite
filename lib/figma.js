	...
	if (typeof JS === 'object' && JS !== null) {
		for (let objectKey in JS) {
			cssString +=
				...
				': ' +
				JS[objectKey] +
				//add px to end if it's one of the above types
...
//fix this
//fix this
//fix this it doesn't work, figma's image thing has changed
async function imageURL(hash) {
	...
	return `url(https://s3-alpha-sig.figma.com/img/${squash.substring(
		0,
		4
...
function expandChildren(
	...
	if (children) {
		for (let i = 0; i < children.length; i++) {
			...
			// 	//if it's bottom-aligned, add to maxChildren?
			// 	maxChildren.push(child);
			// } else if (child.constraints && child.constraints.vertical === 'TOP') {
			// 	//if it's top-aligned, add to maxChildren?
			// 	minChildren.push(child);
			// } else {
...
const createComponent = (component, imgMap, imageFills, componentMap) => {
	...
	const visitNode = (node, parent, lastVertical, indent) => {
		...
		//maybe shouldn't just blanket this lol
		...
		if (cHorizontal === 'LEFT_RIGHT') {
			...
			styles.position = 'absolute';
			styles.left = 'calc(50% - ' + bounds.width / 2 + 'px)';
			if (bounds != null) {
			}
		...
		if (cVertical === 'TOP_BOTTOM') {
			...
			// styles.top = `calc(50% - ${bounds.height / 2})`;
			styles.position = 'absolute';
			styles.top = 'calc(50% - ' + bounds.height / 2 + 'px)';
			if (bounds != null) {
				// styles.marginTop = bounds.top - bounds.bottom;
				// styles.top = bounds.top - bounds.bottom;
			}
		...
		function printDiv(styles, outerStyle, indent) {
			print(
				`<div style="${JSToCSS(outerStyle)}" className="${outerClass}">`,
				indent
			);
			print(`  <div`, indent);
		...
		//here's where it starts printing
		if (parent != null) {
			//if it's not the parent frame
			printDiv(styles, outerStyle, indent);
		} else {
			//if it's the parent frame
			print(
		...
		if (node.id !== component.id && node.name.charAt(0) === '#') {
			//if it's the frame? or if it doesn't match?
			// print(
			...
			createComponent(node, imgMap, componentMap); //recurse and hit createComponent again;
			...
			print(`    <div class="container" style="height:100%;">`, indent); // is this the first div? hmm maybe not
			let first = true;
			...
			// }
			if (content != null) {
				//this is where the content gets printed, like stuff in the span
		...
		if (parent != null) {
			//if it's not the parent, close out?
			print(`  </div>`, indent);
			print(`</div>`, indent);
		} else {
